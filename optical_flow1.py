...
if __name__ == '__main__':
    import sys
    count=0
    cam = cv2.VideoCapture('highway_110.mp4')
    ret, prev = cam.read()
	...
	flow = cv2.calcOpticalFlowFarneback(prevgray, gray, None, 0.6, 3, 10, 2, 4, 1.1, 0)
	prevgray = gray
        #cv2.namedWindow("flow", cv2.WINDOW_NORMAL)
        both = np.vstack((img, draw_flow(gray, flow)))
        cv2.namedWindow("imgc", cv2.WINDOW_NORMAL)
        cv2.resizeWindow("imgc", 1460, 960)
        cv2.line(both,(20,750),(1900,750),(0,0,255),5)
        cv2.putText(both,str(count), (50,950), cv2.FONT_HERSHEY_SIMPLEX, 3, (0,0,255),5)
